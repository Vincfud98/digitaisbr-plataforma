...
import { Tag, Button, Input, Select, Typography, Card, Row, Col, Space, message, Modal, Form, Segmented, Badge, Avatar, Upload } from 'antd';
import {
  CommentOutlined, SearchOutlined, LikeOutlined, MessageOutlined,
    ...
    <div>
        ...
        <Col xs={24} lg={8}>
          ...
          <Card size="small" style={{ borderRadius: 8 }} title="Sobre a Comunidade">
            ...
            <div style={{ marginTop: 10, display: 'flex', gap: 8 }}>
              <Tag color="blue">{topics.length} posts</Tag>
              <Tag color="green">{new Set(topics.map((t) => t.authorId)).size} membros</Tag>
              <Tag color="red">{topics.reduce((s, t) => s + t.likes, 0)} curtidas</Tag>
            </div>
          </Card>
        </Col>
      </Row>
      ...
      <Modal title={null} open={formOpen} onCancel={() => { setFormOpen(false); form.resetFields(); }} onOk={() => form.submit()} okText="Publicar" cancelText="Cancelar" width={600}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, paddingTop: 8 }}>
          ...
          <div>
            <Text strong style={{ fontSize: 15 }}>{user?.name || 'Associado'}</Text>
